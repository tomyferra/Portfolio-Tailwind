    ...
    <div className="max-w-4xl w-full border rounded-lg shadow bg-gray-800 border-gray-400 relative">
      ...
      <div className="p-5">
        <h5 className="mb-2 text-2xl font-bold tracking-tight text-white flex items-center gap-2">
          {name}
          {isStar && <Trophy className="text-yellow-500" size={20} />}
        </h5>
      ...
      <div className="text-center items-center">
        {technologies.map((tech, index) =>
          <p key={index} className="inline-block rounded-full p-0.5 px-2 m-2 text-center bg-slate-500 text-[#00df9a]">{tech}</p>
        )}
      </div>
    </div>
  );
